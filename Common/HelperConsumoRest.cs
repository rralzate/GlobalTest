...
namespace Common
{
    ...
    public class HelperConsumoRest
    {
        ...
        {
            if (!string.IsNullOrEmpty(respuestaApi) && !respuestaApi.StartsWith("Error:"))
            {
                objPeticionRespuesta = JsonConvert.DeserializeObject<T>(respuestaApi);
            }
            else
            {
                if (respuestaApi != null && respuestaApi.StartsWith("Error:"))
                {
        ...
        private static string ResponseMessage(HttpResponseMessage response)
        {
            ...
            {
                respuestaRest = $"Error: {response.StatusCode} ({response.ReasonPhrase})";
            }
